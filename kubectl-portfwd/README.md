...
explicitly with a `svc/`, `pod/`, `deploy/`, `sts/`, `ds/`, `job/`, or `cronjob/` segment (aliases: `service`/`services`,
`po`/`pods`, `deployment`/`deployments`, `statefulset`/`statefulsets`, `daemonset`/`daemonsets`, `jobs`, `cronjobs`, `cj`); there is no implicit default.
...
ns-kubectl-portfwd/job/data-migration:5432::50005
ns-kubectl-portfwd/cronjob/nightly-backup:5432::50006
```

Each line uses the same format as `--target`: `[namespace/]type/name[:remote_port][::local_port]`, where `type` is `svc`, `pod`, `deploy`, `sts`, `ds`, `job`, or `cronjob` (aliases: `service`/`services`, `po`/`pods`, `deployment`/`deployments`, `statefulset`/`statefulsets`, `daemonset`/`daemonsets`, `jobs`, `cronjobs`, `cj`).

When ports are omitted, the remote port is read from the target (single-port services/pods/deployments/statefulsets/daemonsets/jobs/cronjobs only) and the local port is chosen automatically.
