...
async def test_fetch_services_for_namespaces_combines_services_from_each_namespace():
    ...
    class FakeServiceRepo:
        async def list(self, namespace: str) -> list[Service]:
            return {"ns-1": [svc_a], "ns-2": [svc_b]}.get(namespace, [])

    api = _make_api(FakeServiceRepo())
    specs = await fetch_services_for_namespaces(["ns-1", "ns-2"], api)
...
async def test_fetch_services_for_namespaces_returns_empty_for_empty_namespaces():
    ...
    api = _make_api(FakeServiceRepo())
    assert await fetch_services_for_namespaces([], api) == []


async def test_fetch_services_for_namespaces_sorts_and_expands_multi_port_services():
    ...
    api = _make_api(FakeServiceRepo())
    specs = await fetch_services_for_namespaces(["ns-b", "ns-a"], api)
...
async def test_fetch_pods_for_namespaces_flattens_declared_container_ports():
    ...
    class FakePodRepo:
        async def list(self, namespace: str) -> list[Pod]:
            return {
    ...
    specs = await fetch_pods_for_namespaces(["ns-1", "ns-2"], api)
...
async def test_fetch_targets_combines_services_and_pods():
...
async def test_fetch_targets_honors_selected_kinds():
    ...
    pods_only = await fetch_targets_for_namespaces(
        ["ns-1"], api, kinds=[TargetKind.POD]
    )
    assert {(s.target.kind, s.target.name) for s in pods_only} == {
        (TargetKind.POD, "worker")
    }

    svcs_only = await fetch_targets_for_namespaces(
        ["ns-1"], api, kinds=[TargetKind.SERVICE]
    )
...
async def test_fetch_deployments_for_namespaces_flattens_declared_container_ports():
    ...
    class FakeDeploymentRepo:
        async def list(self, namespace: str) -> list[Deployment]:
            return {
    ...
    specs = await fetch_deployments_for_namespaces(["ns-1", "ns-2"], api)
...
async def test_fetch_targets_includes_deployments():
...
async def test_fetch_targets_honors_deployment_kind_filter():
    ...
    specs = await fetch_targets_for_namespaces(
        ["ns-1"], api, kinds=[TargetKind.DEPLOYMENT]
    )
...
async def test_fetch_statefulsets_for_namespaces_flattens_declared_container_ports():
    ...
    class FakeStatefulSetRepo:
        async def list(self, namespace: str) -> list[StatefulSet]:
            return {
    ...
    specs = await fetch_statefulsets_for_namespaces(["ns-1", "ns-2"], api)
...
async def test_fetch_targets_includes_statefulsets():
...
async def test_fetch_targets_honors_statefulset_kind_filter():
    ...
    api = _make_api(service=FakeServiceRepo(), statefulset=FakeStatefulSetRepo())
    specs = await fetch_targets_for_namespaces(
        ["ns-1"], api, kinds=[TargetKind.STATEFULSET]
    )
...
async def test_fetch_daemonsets_for_namespaces_flattens_declared_container_ports():
    ...
    class FakeDaemonSetRepo:
        async def list(self, namespace: str) -> list[DaemonSet]:
            return {
    ...
    specs = await fetch_daemonsets_for_namespaces(["ns-1", "ns-2"], api)
...
async def test_fetch_targets_includes_daemonsets():
...
async def test_fetch_targets_honors_daemonset_kind_filter():
    ...
    api = _make_api(service=FakeServiceRepo(), daemonset=FakeDaemonSetRepo())
    specs = await fetch_targets_for_namespaces(
        ["ns-1"], api, kinds=[TargetKind.DAEMONSET]
    )
...
async def test_fetch_replicasets_for_namespaces_flattens_declared_container_ports():
    ...
    class FakeReplicaSetRepo:
        async def list(self, namespace: str) -> list[ReplicaSet]:
            return {
    ...
    specs = await fetch_replicasets_for_namespaces(["ns-1", "ns-2"], api)
...
async def test_fetch_targets_includes_replicasets():
...
async def test_fetch_targets_honors_replicaset_kind_filter():
    ...
    api = _make_api(service=FakeServiceRepo(), replicaset=FakeReplicaSetRepo())
    specs = await fetch_targets_for_namespaces(
        ["ns-1"], api, kinds=[TargetKind.REPLICASET]
    )
